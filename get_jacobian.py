...
# -----------------------------
model = pin.buildModelFromUrdf("xml_files/biped.urdf")
...
# Joint state (example)
# -----------------------------
q = np.array([0.5, 0.0, 0.5, 0.0])   # use your desired q

# -----------------------------
# Correct Jacobian function
# -----------------------------
def get_pos_2d_jacobians(q):
    """
    ...
    right_cols = joint_velocity_indices(model, "right_hip") + joint_velocity_indices(model, "right_knee")
    left_cols  = joint_velocity_indices(model, "left_hip")  + joint_velocity_indices(model, "left_knee")

    J_r = Jr_lin[[0,2], :][:, right_cols]
