...
K_xi = -1.0 # tuning variable
K_pz = 5
K_pt = -8
dx_des_ = 0.0 # overwritten by keyboard or controller input
dy_des_ = 0.0 # overwritten by keyboard or controller input
dy_rocking = 0.1
...
class Walking_controller():
    ...
    def step(self, contact_l, contact_r, q, dx_des, dy_des, t):
        ...
        if self.stance["stance"] == "Right":
            ...
            p_y = np.clip(p_y, a_min=0.15, a_max=0.6)
        else:
            if p_y > 0.0:
                e_step_down += 2*p_y
            elif p_y < -0.5:
                e_step_down += -2*(p_y+0.5)
            p_y = np.clip(p_y, a_min=-0.6, a_max=-0.15)
        if abs(p_x) > 0.5:
        ...
        vel_vector_ = np.array([[v_t[0,0]], [v_t[1,0]], [v_t[2,0]]])
        ...
        vel_swing_ = np.array([[v_s[0,0]], [v_s[1,0]], [v_s[2,0]]])
        ...
        if self.stance["stance"] == "Right":
            return {"right_hip_y": dq_stance[0,0], "right_hip": dq_stance[1,0], "right_knee": dq_stance[2,0], "left_hip_y": dq_swing[0,0], "left_hip": dq_swing[1,0], "left_knee": dq_swing[2,0]}
        else:
            return {"left_hip_y": dq_stance[0,0], "left_hip": dq_stance[1,0], "left_knee": dq_stance[2,0], "right_hip_y": dq_swing[0,0], "right_hip": dq_swing[1,0], "right_knee": dq_swing[2,0]}



model = mujoco.MjModel.from_xml_path("xml_files/biped_3d_feet.xml")
...
#controller = utils.XboxController(v_step=0.6, v_side=0.15, yaw_step=2.0)
controller = utils.KeyboardController(v_step=0.6, v_side=0.15, yaw_step=1.0)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        q = np.array([left_hip_y_j, left_hip, left_knee, right_hip_y_j, right_hip, right_knee])
        ...
        data.ctrl[ctrl_r_hip_y] = np.clip(dq_c["right_hip_y"], -5, 5)
        data.ctrl[ctrl_l_hip_y] = np.clip(dq_c["left_hip_y"], -5, 5)
        data.ctrl[ctrl_l_foot] = 0.0
        data.ctrl[ctrl_l_foot] = 0.0
