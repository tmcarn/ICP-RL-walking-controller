...
def swing_leg_controller(p_des_swing, p_swing):
    ...
    return 20*np.array([e_x, 0.5*e_y, e_z])
...
class Walking_controller():
    ...
    def step(self, contact_l, contact_r, q, dx_des, dy_des, t):
        ...
        if self.stance["stance"] == "Right":
            p_y = np.clip(p_y, a_min=0.15, a_max=1.05)
        else:
            p_y = np.clip(p_y, a_min=-1.05, a_max=-0.15)
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        d_right_hip  = get_joint_velocity(model, data, "right_hip")
        d_right_knee = get_joint_velocity(model, data, "right_knee")
        d_left_hip   = get_joint_velocity(model, data, "left_hip")
        d_left_knee  = get_joint_velocity(model, data, "left_knee")
